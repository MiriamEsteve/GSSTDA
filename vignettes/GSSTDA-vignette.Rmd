...
values of the filter functions will be divided and the overlap between them (\code{percent_overlap}). Default are 5 and 40 respectively. It is also 
...
process and can be used with the data to perform other analyses that are not mapper 
...
```{r}
GSSTDA_obj <- GSSTDA(full_data, survival_time, survival_event, case_tag, 
                     gen_select_type, percent_gen_select, num_intervals, 
